...
- `scripts/copy-libs.js` - Postinstall script that copies Three.js and MediaPipe WASM files to `/lib`
...
```javascript
{
  type: 'send-text' | 'send-image' | 'send-tiled-image' | 'send-video' | 'video-control' | 'send-color' | 'cascade-text' | 'cascade-words' | 'clear' | 'effect' | 'show-eyeball' | 'hide-eyeball' | 'eyeball-gaze' | 'config-display',
